    ...
    <div className="hero-wash min-h-screen">
      <header className="sticky top-0 z-40 border-b border-border bg-bg/80 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
          <a className="flex items-center gap-2.5" href="/">
            <span className="display grid size-8 place-items-center rounded-md bg-fg text-lg font-bold text-bg">F</span>
          ...
          <div className="hidden items-center gap-2 sm:flex">
            <a href="#contato" className="inline-flex h-10 items-center gap-2 rounded-md border border-border px-3 text-sm text-fg hover:bg-elevated">
              <MessageCircle className="size-4 text-lime" />
              <span className="hidden lg:inline">(11) 99588-0000</span>
            </a>
            <a className="inline-flex h-10 items-center gap-2 rounded-md border border-border px-3 text-sm text-fg hover:bg-elevated" href="#planos">
              <GraduationCap className="size-4" />
              Já sou aluno
            </a>
            <button type="button" className="hidden h-10 rounded-md bg-lime px-4 text-sm font-semibold text-lime-fg hover:brightness-110 lg:inline-flex lg:items-center">
              Quero assinar
            </button>
          </div>

          <button type="button" className="rounded-md p-2 text-fg md:hidden" aria-label="Abrir menu" onClick={() => setMenuOpen(!menuOpen)}>
        ...
        {menuOpen && (
          <div className="border-t border-border bg-bg px-4 py-4 md:hidden">
            <nav className="flex flex-col gap-3 text-sm">
              ...
              <a href="#contato" className="inline-flex items-center gap-2 text-lime" onClick={() => setMenuOpen(false)}>
      ...
      <main>
        <section className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-2 lg:py-16">
          <div>
            <p className="display one-title text-4xl font-light tracking-tight sm:text-5xl">FORTIFY ONE</p>
            <h1 className="display mt-5 text-3xl font-semibold leading-tight sm:text-4xl">
              Hacking e Cibersegurança<br />do Zero ao Avançado.
            </h1>
            <p className="mt-4 max-w-md text-muted">
              Novidades o ano todo. Tudo numa única formação — labs, CTFs, certificações práticas e comunidade.
            </p>
            <button type="button" className="mt-8 inline-flex h-12 items-center rounded-md bg-lime px-7 text-sm font-bold tracking-wide text-lime-fg hover:brightness-110">
              QUERO ASSINAR AGORA
            </button>
          </div>
          ...
          <div className="card-stack relative mx-auto h-[420px] w-full max-w-[420px] sm:h-[480px]">
            <PosterCard
              ...
              gradient="bg-gradient-to-br from-violet-900 via-purple-800 to-indigo-950"
            />
            <PosterCard
              label="BLUE TEAM"
              title="Defesa ativa"
              className="right-8 top-4 rotate-3"
              gradient="bg-gradient-to-br from-cyan-900 via-teal-800 to-slate-900"
            />
            <PosterCard
              label="10ª EDIÇÃO"
              title="CAPTURE THE FLAG"
              subtitle={<>Prêmio de <span className="text-fg">R$ 15.000</span></>}
              className="left-4 top-0 sm:left-auto sm:right-16"
              gradient="bg-gradient-to-br from-lime-900/80 via-emerald-900 to-zinc-900"
            />
          </div>
        </section>
        ...
        <section id="trilhas" className="border-t border-border py-16">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            <p className="text-xs font-medium tracking-[0.2em] text-violet uppercase">Confira as trilhas</p>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {trilhas.map((t) => (
                <div key={t} className="flex items-center gap-3 rounded-lg border border-border bg-surface px-4 py-3">
        ...
        <section id="certificacoes" className="py-16">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            ...
            <p className="mt-2 max-w-xl text-muted">Comprove ao mercado sua expertise e receba o kit de certificação em casa.</p>
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {certificacoes.map((c) => (
                <article key={c.code} className="rounded-xl border border-border bg-surface p-5">
        ...
        <section id="ctf" className="border-y border-border py-16">
          <div className="mx-auto grid max-w-6xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-2">
            <div>
              <p className="text-xs font-medium tracking-[0.2em] text-lime uppercase">Capture the Flag</p>
              <h2 className="display mt-2 text-3xl font-semibold">Venha ser o novo campeão</h2>
              <p className="mt-3 text-muted">
                Dois campeonatos por ano, cenários realistas e R$ 15.000 em PIX para o primeiro lugar de cada edição. Treine como no mercado.
              </p>
              <ul className="mt-6 space-y-2 text-sm">
                {["Web, infra, wireless e forense no mesmo evento", "Hall da fama com os campeões de cada edição", "Incluso em qualquer plano Fortify One"].map((item) => (
            ...
            <div className="relative overflow-hidden rounded-xl border border-border">
              <div className="h-80 w-full bg-gradient-to-br from-lime-900/60 via-emerald-900 to-zinc-950" />
        ...
        <section className="py-16">
          <div className="mx-auto max-w-6xl px-4 sm:px-6">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              {depoimentos.map((d) => (
                <blockquote key={d.nome} className="rounded-xl border border-border bg-surface p-5">
          ...
          <div className="mx-auto max-w-6xl px-4 text-center sm:px-6">
            ...
            <div className="mt-10 grid items-stretch gap-4 text-left md:grid-cols-3">
              <article className="relative rounded-xl border border-border bg-surface p-6">
                ...
                <ul className="mt-6 space-y-2 text-sm">
                  {["Acesso à grade completa", "Labs práticos", "Certificado de conclusão", "Acesso por 12 meses"].map((f) => (
                ...
                <button type="button" className="mt-7 h-12 w-full rounded-md border border-border text-sm font-semibold hover:bg-elevated">Começar</button>
              </article>

              <article className="relative rounded-xl border border-lime/50 bg-surface p-6 md:-translate-y-2">
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-lime px-3 py-1 text-[11px] font-bold text-lime-fg">MAIS POPULAR</span>
                ...
                <ul className="mt-6 space-y-2 text-sm">
                  {["Tudo do Básico", "Acesso vitalício", "Comunidade exclusiva", "Atualizações futuras", "Mentoria em grupo"].map((f) => (
                    <li key={f} className="flex gap-2">
                      <Check className="mt-0.5 size-4 shrink-0 text-lime" />
                      {f}
                    </li>
                ...
                <button type="button" className="mt-7 h-12 w-full rounded-md bg-lime text-sm font-semibold text-lime-fg hover:brightness-110">Quero esse</button>
              </article>
              ...
              <article className="relative rounded-xl border border-border bg-surface p-6">
                ...
                <ul className="mt-6 space-y-2 text-sm">
                  {["Tudo do Completo", "Mentoria 1:1 (4 sessões)", "Revisão de projetos", "Kit certificado físico", "Banco de talentos"].map((f) => (
                    <li key={f} className="flex gap-2">
                      <Check className="mt-0.5 size-4 shrink-0 text-lime" />
                      {f}
                    </li>
              ...
              {faqs.map((faq, i) => (
                <div key={faq.q}>
                  ...
                  >
                    ...
                    <ChevronDown className={`size-4 shrink-0 text-muted transition-transform duration-200 ${openFaq === i ? "rotate-180" : ""}`} />
                  </button>
                  {openFaq === i && (
        ...
        <section className="px-4 pb-20 sm:px-6">
          <div className="mx-auto max-w-4xl rounded-xl border border-border bg-elevated px-6 py-12 text-center">
            ...
            <h2 className="display mt-4 text-3xl font-semibold">Experimente 7 dias ou tenha seu dinheiro de volta</h2>
            <p className="mx-auto mt-3 max-w-lg text-muted">
              A maior formação prática de cibersegurança para quem quer sair do tutorial e atuar de verdade.
            </p>
            <button type="button" className="mt-8 inline-flex h-12 items-center rounded-md bg-lime px-8 text-sm font-bold text-lime-fg hover:brightness-110">
              QUERO ASSINAR AGORA
            </button>
          </div>
        </section>
      ...
      <footer id="contato" className="border-t border-border py-10">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-6 px-4 sm:flex-row sm:items-center sm:px-6">
          <div className="flex items-center gap-2">
            <span className="display grid size-7 place-items-center rounded-md bg-fg text-sm font-bold text-bg">F</span>
            <span className="display font-semibold">FORTIFY</span>
          </div>
          <a href="https://wa.me/5511995880000" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-muted hover:text-fg">
            <MessageCircle className="size-4 text-lime" />
