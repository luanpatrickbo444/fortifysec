...
export default async function ChallengePage({params,searchParams}:{params:Promise<{slug:string}>,searchParams:Promise<{result?:string}>}){
 ...
 const {data:c}=await supabase.from('challenges').select('id,title,slug,description,category,difficulty,xp_reward,briefing').eq('slug',slug).eq('published',true).maybeSingle()
 if(!c)notFound()
