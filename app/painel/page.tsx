...
async function safeData<T>(label: string, operation: () => PromiseLike<QueryResult<T>>, fallback: T): Promise<T> {
  try {
    ...
    if (result?.error) {
      console.error(`[painel:${label}]`, result.error)
      return fallback
    }
    return result?.data ?? fallback
  } catch (error) {
    console.error(`[painel:${label}]`, error)
    return fallback
  }
}
  ...
  return <>
    ...
    <section className="operator-command-banner">
      ...
      <div className="command-next">
        {runningSession && runningLab ? <><span className="status-orb online"/><div><small>LAB EM EXECUÇÃO</small><strong>{runningLab.title}</strong></div><a className="btn" href={`/painel/labs/${runningLab.slug}`}>RETOMAR LAB →</a></>
    ...
    <section className="command-grid">
      ...
      <article className="card command-panel">
        <div className="panel-head"><div><span className="section-index">CYBER RANGE</span><h2>Labs disponíveis</h2></div><a className="tag green" href="/painel/labs">RANGE →</a></div>
        <div className="command-mini-grid">{labs.slice(0, 4).map((lab) => <a href={`/painel/labs/${lab.slug}`} className="command-mini-card" key={lab.id}><Boxes size={17}/><div><strong>{lab.title}</strong><span>{lab.difficulty} · {lab.estimated_minutes} min</span></div></a>)}{!labs.length && <div className="empty-inline">Nenhum Lab publicado.</div>}</div>
      </article>

      <article className="card command-panel">
        <div className="panel-head"><div><span className="section-index">MISSIONS</span><h2>Challenges</h2></div><a className="tag green" href="/painel/desafios">TODOS →</a></div>
        <div className="command-mini-grid">{challenges.slice(0, 4).map((challenge) => <a href={`/painel/desafios/${challenge.slug}`} className="command-mini-card" key={challenge.id}><Swords size={17}/><div><strong>{challenge.title}</strong><span>{challenge.category} · {challenge.difficulty} · +{challenge.xp_reward} XP</span></div></a>)}{!challenges.length && <div className="empty-inline">Nenhum Challenge publicado.</div>}</div>
      </article>
...
export default async function PainelHomePage() {
  try {
    ...
    console.error('[painel:fatal]', error)
    ...
    return <>
      ...
      <div className="dashboard-grid">
        ...
        <a className="stat-card" href="/painel/labs"><Boxes size={18}/><small>LABS</small><div className="stat">ABRIR</div></a>
        <a className="stat-card" href="/painel/desafios"><Swords size={18}/><small>CHALLENGES</small><div className="stat">ABRIR</div></a>
