 ...
 return <DashboardShell admin={profile?.role==='admin'}>
   ...
   <div className="lab-workspace-grid">
     ...
     <aside className="workspace-side">
       ...
       <div className="card session-control"><span className="section-index">SESSION CONTROL</span><h3>{session?'Ambiente em execução':'Pronto para iniciar'}</h3><p className="muted">{session?'Encerre a sessão quando terminar para liberar os recursos do range.':'O servidor validará sua conta e matrícula antes de provisionar o alvo.'}</p>{session?<form action={stopLabAction}><input type="hidden" name="lab_id" value={lab.id}/><input type="hidden" name="slug" value={lab.slug}/><SubmitButton className="btn danger full-btn" idleLabel="ENCERRAR SESSÃO" pendingLabel="ENCERRANDO..."/></form>:<form action={startLabAction}><input type="hidden" name="lab_id" value={lab.id}/><input type="hidden" name="slug" value={lab.slug}/><SubmitButton className="btn full-btn" idleLabel="INICIAR LAB →" pendingLabel="PROVISIONANDO LAB..."/></form>}<div className="security-note"><Power size={14}/><span>Endpoints só aparecem após autorização server-side.</span></div></div>
     </aside>
   </div>
 </DashboardShell>
}
