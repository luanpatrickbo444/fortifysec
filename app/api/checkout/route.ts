...
export async function POST(request:Request){
  ...
  const [{data:course},{data:profile},{data:existing}]=await Promise.all([
    admin.from('courses').select('id,title,slug,price_cents,published').eq('id',courseId).eq('published',true).maybeSingle(),
