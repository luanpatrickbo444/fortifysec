...
export default async function CompanyJobs({searchParams}:{searchParams:Promise<{status?:string;erro?:string;criada?:string;salvo?:string}>}){
  ...
  return <CompanyShell companyName={company.name} verified={company.verified}>
    ...
    <div className="job-filter-tabs">
      <Link prefetch={false} className={activeFilter==='all'?'active':''} href="/empresa/job-console">TODAS <b>{counts.all}</b></Link>
      <Link prefetch={false} className={activeFilter==='published'?'active':''} href="/empresa/job-console?status=published">PUBLICADAS <b>{counts.published}</b></Link>
      <Link prefetch={false} className={activeFilter==='draft'?'active':''} href="/empresa/job-console?status=draft">RASCUNHOS <b>{counts.draft}</b></Link>
      <Link prefetch={false} className={activeFilter==='closed'?'active':''} href="/empresa/job-console?status=closed">ENCERRADAS <b>{counts.closed}</b></Link>
    </div>
    ...
    <section className="employer-job-grid">{filtered.map((j:any)=><article className="employer-job-card" key={j.id}>
      ...
      <div className="employer-job-actions"><Link prefetch={false} className="btn secondary small" href={`/empresa/job-console/${j.id}/editar`}><Edit3 size={13}/> EDITAR</Link><Link className="btn secondary small" href="/empresa/candidatos">CANDIDATOS</Link></div>
