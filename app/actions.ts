...
function safeAdminReturn(value:FormDataEntryValue|null,fallback='/admin'){const v=String(value||'');return v.startsWith('/admin/')||v.startsWith('/admin?')||v==='/admin'?v:fallback}
function withQueryParam(url:string,key:string,value:string){const join=url.includes('?')?'&':'?';return `${url}${join}${encodeURIComponent(key)}=${encodeURIComponent(value)}`}
function routePath(url:string){return url.split('?')[0]||'/admin'}
...
export async function adminCreateLessonAction(formData:FormData){
 await requireAdmin();const admin=createAdminClient();const courseId=String(formData.get('course_id')||'');const moduleId=String(formData.get('module_id')||'')||null;const title=String(formData.get('title')||'').trim();const position=Number(formData.get('position')||1);const returnTo=safeAdminReturn(formData.get('return_to'),'/admin/aulas');if(!courseId||!title||position<1)redirect(withQueryParam(returnTo,'erro','dados'));
 if(moduleId){const {data:module}=await admin.from('course_modules').select('id').eq('id',moduleId).eq('course_id',courseId).maybeSingle();if(!module)redirect(withQueryParam(returnTo,'erro','modulo'))}
 const {error}=await admin.from('lessons').insert({course_id:courseId,module_id:moduleId,title,summary:String(formData.get('summary')||'').trim(),content:String(formData.get('content')||''),video_url:safeExternalUrl(String(formData.get('video_url')||'').trim()),position,xp_reward:Math.max(0,Number(formData.get('xp_reward')||10)),published:asBool(formData.get('published'))});if(error)redirect(withQueryParam(returnTo,'erro','salvar'));revalidatePath('/admin/aulas');revalidatePath(routePath(returnTo));revalidatePath('/painel/cursos');redirect(withQueryParam(returnTo,'criada','1'))
}

export async function adminToggleCourseAction(formData:FormData){await requireAdmin();const admin=createAdminClient();const courseId=String(formData.get('course_id')||'');const published=String(formData.get('published')||'false')==='true';await admin.from('courses').update({published:!published}).eq('id',courseId);revalidatePath('/admin');revalidatePath('/painel/cursos')}
