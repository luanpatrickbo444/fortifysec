...
- `GET /healthz` (público, mínimo)
- `GET /health` (Bearer; capacidade detalhada)
...
O provider expõe no `GET /health` autenticado `max_sessions`, `available_slots` e `max_sessions_per_user`. Configure:
