...
const DYNAMIC_FLAG_SECRET = String(process.env.CTF_DYNAMIC_FLAG_SECRET || '')
const NODE_NAME = String(process.env.RANGE_NODE_NAME || 'range-node')
...
function dynamicFlag({ sessionId, userId, labId }) {
  if (!DYNAMIC_FLAG_SECRET) return null
  const digest = createHmac('sha256', DYNAMIC_FLAG_SECRET)
    .update(`${sessionId}:${userId}:${labId}`)
    .digest('hex')
    .slice(0, 32)
  return `FORTIFY{${digest}}`
}
function estimatedActiveCostCents() {
  const now = Date.now()
  return activeSessions().reduce((total, s) => {
    const perHour = Math.max(0, Number(s.costCentsPerHour || 0))
    const started = new Date(s.createdAt || now).getTime()
    const hours = Math.max(0, now - started) / 3_600_000
    return total + Math.ceil(perHour * hours)
  }, 0)
}
...
async function provisionLocalSession({ id, userId, labId, expiresAt, def, flag }) {
  const local = await provisionLocalDocker({ sessionId: id, userId, labId, expiresAt, def, dynamicFlag: flag })
  const session = { id, provider: 'local_docker', status: 'running', userId, labId, targetUrl: local.targetUrl, expiresAt, createdAt: new Date().toISOString(), local, costCentsPerHour: Number(def.estimated_cost_cents_per_hour || 0) }
  state.sessions[id] = session
  await saveState()
  return localResponse(session)
}

async function provisionGcpSession({ id, userId, labId, expiresAt, def, slot, flag }) {
  ...
  try {
    gcp = await provisionGcpVm({ sessionId: id, userId, labId, expiresAt, def, dynamicFlag: flag })
    ...
    const session = {
      ...
      token, config, expiresAt, gcp, egressInterface, def: { scheme: def.scheme || null, port: def.port || null }, createdAt: new Date().toISOString(), costCentsPerHour: Number(def.estimated_cost_cents_per_hour || 0),
    }
    state.sessions[id] = session
    await saveState()
...
async function provisionSession({ userId, labId, ttlMinutes }) {
  ...
  if (active.length + provisioningKeys.size >= MAX_ACTIVE_SESSIONS) {
    throw Object.assign(new Error('range capacity reached'), { statusCode: 503 })
  }
  if (activeSessionsForUser(userId).length >= MAX_SESSIONS_PER_USER) {
    throw Object.assign(new Error('user concurrent session limit reached'), { statusCode: 429 })
  }
  ...
  try {
    ...
    const flag = dynamicFlag({ sessionId: id, userId, labId })
    if (type === 'gcp') {
      slot = allocateSlot()
      return await provisionGcpSession({ id, userId, labId, expiresAt, def, slot, flag })
    }
    return await provisionLocalSession({ id, userId, labId, expiresAt, def, flag })
  } finally {
    releaseSlot(slot)
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') {
      ...
      return json(res, 200, {
        ...
        available: Math.max(0, MAX_ACTIVE_SESSIONS - running.length - provisioningKeys.size),
        ...
        dynamic_flags: Boolean(DYNAMIC_FLAG_SECRET),
    ...
    if (req.method === 'GET' && url.pathname === '/stats') {
      ...
      const running = activeSessions()
      return json(res, 200, {
        ok: true,
        node: NODE_NAME,
        running: running.length,
        provisioning: provisioningKeys.size,
        capacity: MAX_ACTIVE_SESSIONS,
        available: Math.max(0, MAX_ACTIVE_SESSIONS - running.length - provisioningKeys.size),
        max_sessions_per_user: MAX_SESSIONS_PER_USER,
        max_ttl_minutes: MAX_TTL,
        dynamic_flags: Boolean(DYNAMIC_FLAG_SECRET),
        estimated_active_cost_cents: estimatedActiveCostCents(),
        providers: {
          local_docker: running.filter(s => s.provider === 'local_docker').length,
          gcp: running.filter(s => s.provider === 'gcp').length,
        },
      })
    }
