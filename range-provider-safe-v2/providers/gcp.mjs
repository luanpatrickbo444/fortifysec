...
function metadataItems(def, { sessionId, expiresAt, dynamicFlag }) {
  const items = []
  const scripts = []
  if (dynamicFlag) {
    const encoded = Buffer.from(String(dynamicFlag), 'utf8').toString('base64')
    scripts.push(`install -d -m 700 /opt/fortify\nprintf '%s' '${encoded}' | base64 -d > /opt/fortify/flag\nchmod 600 /opt/fortify/flag`)
  }
  if (def.startup_script) scripts.push(String(def.startup_script))
  if (scripts.length) items.push({ key: 'startup-script', value: `#!/bin/bash\nset -e\n${scripts.join('\n')}` })
...
export async function provisionGcpVm({ sessionId, userId, labId, expiresAt, def, dynamicFlag = null }) {
  ...
  const instanceResource = {
    ...
    serviceAccounts: [],
    metadata: { items: metadataItems(def, { sessionId, expiresAt, dynamicFlag }) },
